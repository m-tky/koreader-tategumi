...
function ReaderTypeset:onToggleVerticalReading(toggle)
    ...
    if toggle then
        after_open_callback = function()
            UIManager:show(InfoMessage:new{
                text = _("Vertical reading may not work well with books styled specifically for horizontal reading."),
                timeout = 5,
                icon = "notice-warning",
            })
        end
...
function ReaderTypeset:addToMainMenu(menu_items)
    ...
    if document_settings and document_settings.sub_item_table then
        table.insert(document_settings.sub_item_table, {
            ...
            help_text = _("Read vertically from right to left. Books styled for horizontal reading may not lay out as expected."),
        })
    end
end
